...
class ScanOss(
    ...
    override fun scanPathInternal(path: File): ScanSummary {
        ...
        if (path.isDirectory) {
            scanner.scanDirectory(path.absolutePath, null, "", ScanFormat.plain, resultFile.absolutePath)
        } else if (path.isFile) {
            scanner.scanFileAndSave(path.absolutePath, null, "", ScanFormat.plain, resultFile.absolutePath)
        }
